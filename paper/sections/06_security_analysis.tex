...
\begin{equation}
    \hat{\mathbf{y}}_t = \mathbf{W}_{\mathrm{head}} \cdot \mathrm{LayerNorm}\left( \sum_{i=1}^t \alpha_{t,i} \mathbf{v}_i \mathbf{W}_O \right)
\end{equation}
...
\begin{equation}
    \mathbf{h}_t = \bar{\mathbf{A}}_t \mathbf{h}_{t-1} + \bar{\mathbf{B}}_t \tilde{\mathbf{x}}_t, \quad \bar{\mathbf{A}}_t = \exp(-\Delta t(\tilde{\mathbf{x}}_t) \cdot \mathbf{A})
\end{equation}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/fig20d_adversarial_jailbreak_unlearning.png}
    \caption{\textbf{Adversarial Probe Leakage vs. Knowledge Retention.} Comparison of gradient-ascent unlearning, Mamba decay, and \textsc{Hokie-LM} CAFE orthogonal projection ($\mathbf{P}_\perp$) across 5 neural probe architectures.}
    \label{fig:adversarial_unlearning}
\end{figure}
